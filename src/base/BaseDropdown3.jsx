...
import { searchByLangAndSortService, searchByLanguage } from "../API/services/other/SearchService";
import { headers } from "../API/tokens/tokens";


const BaseDropDown3 = (props) => {
    let catName = props.name;
    let sortName = props.sort;
  const getValue = (name) => {
    console.log(name);
    console.log("filterValue " + name + catName);
    searchByLanguage(catName , name , "" , headers).then((res) => {
        console.log(res);
    })
